...
(function () {
  ...
  function fitHero() {
    ...
    /* 0.42 is a desktop number: the base size there is ~70px, so it still
       lands around 29px. On a phone the base is already ~29px, and 0.42
       would shrink the pinned headline to 12px. */
    var cap = window.matchMedia('(max-width: 620px)').matches ? 0.78 : 0.42;
    var scale = natural > 0 ? Math.min(cap, room / natural) : cap;
